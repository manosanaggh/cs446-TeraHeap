	...
	while (q < t) {
		...
		if (oop(q)->is_gc_marked()) 
		{
			...
			if (EnableTeraCache && tc_policy(q, size)) {
				...
				HeapWord* region_top = (HeapWord*) Universe::teraCache()->tc_region_top(oop(q), size);
				assertf(Universe::teraCache()->tc_check(oop(region_top)), "Pointer from teraCache is not valid");
