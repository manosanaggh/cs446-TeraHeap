...
std::vector<unsigned int> PSParallelCompact::tc_threads_from; //paralle old
std::vector<unsigned int> PSParallelCompact::tc_threads_to; //parallel old
pthread_barrier_t      	PSParallelCompact::tc_barrier; //parallel old
pthread_t              	PSParallelCompact::tc_threads[MAX_NUM_THREADS]; //parallel old
unsigned int           	rest; //parallel old
Monitor			*PSParallelCompact::map_monitor = new Monitor(1, "map_monitor", false);
...
#endif  // #ifdef ASSERT
unsigned int num_of_gcs = 1;
unsigned int k = -1;
double total_gc_time_par = 0;
unsigned int objects_left = 0;
unsigned long long total_phase1=0,total_phase2=0,total_phase3=0, total_phase4=0, phase1=0, phase2=0, phase3=0,phase4=0,total_major_gcs=0;
...
bool PSParallelCompact::tc_policy(HeapWord *q, size_t size) {
...
{
/*#if DEBUG_PAR_OLD_MARKING
                        if (EnableTeraCache) {
                                std::cerr <<"[ADD_OBJ]"
                                        << " | OBJECT = " << addr
                                        << " | MARKED = " << PSParallelCompact::is_marked(oop(addr))
                                        << " | TERA = "   << oop(addr)->get_obj_state()
                                        << std::endl;
                        }
#endif*/
                                    ...
                                    HeapWord** target_next)
{
  ...
  while (cur_region < end_region) {
    ...
    /*if(!oop(region_to_addr(cur_region))->is_tera_cache())*/_region_data[cur_region].set_destination(dest_addr); //tc parallel old
    //else _region_data[cur_region].set_destination(region_to_addr(cur_region)); //tc parallel old
...
HeapWord* ParallelCompactData::calc_new_pointer(HeapWord* addr) {
  assertf(addr != NULL, "Should detect NULL oop earlier");
  assertf(PSParallelCompact::gc_heap()->is_in(addr), "not in heap");
  assertf(PSParallelCompact::mark_bitmap()->is_marked(addr), "not marked"); 
...
void PSParallelCompact::AdjustPointerClosure::do_oop(oop* p)       { adjust_pointer(NULL,p); }
void PSParallelCompact::AdjustPointerClosure::do_oop(narrowOop* p) { adjust_pointer(NULL,p); }
...
/*#if DISABLE_TC_PAR_OLD
	if(oop(p)->is_tera_cache() || Universe::teraCache()->tc_check(oop(p))){std::cerr << "fill_with_live_objects" << std::endl;}
    if(!oop(p)->is_tera_cache() && !Universe::teraCache()->tc_check(oop(p))){ //tc parallel old
#endif*/
...
PSParallelCompact::provoke_split_fill_survivor(SpaceId id)
{
  ...
  if (space->is_empty()) {
    ...
    while (b + obj_len <= t) {
/*#if DISABLE_TC_PAR_OLD
	    if(oop(b)->is_tera_cache() || Universe::teraCache()->tc_check(oop(b))){std::cerr << "provoke_split_full_survivor" << std::endl;}
       if(!oop(b)->is_tera_cache() && !Universe::teraCache()->tc_check(oop(b))){ //tc parallel old
#endif*/
...
/*#if DISABLE_TC_PAR_OLD
      if(oop(cur_addr)->is_tera_cache() || Universe::teraCache()->tc_check(oop(cur_addr))){std::cerr << "provoke_split" << std::endl;}
	if(!oop(cur_addr)->is_tera_cache() && !Universe::teraCache()->tc_check(oop(cur_addr))){ //tc parallel old
#endif*/
...
#endif  // #ifdef _LP64
/*#if DISABLE_TC_PAR_OLD
    if(oop(obj_beg)->is_tera_cache() || Universe::teraCache()->tc_check(oop(obj_beg))){std::cerr << "fill_dense_prefix_end" << std::endl;}
	if(!oop(obj_beg)->is_tera_cache() && !Universe::teraCache()->tc_check(oop(obj_beg))){ //tc parallel old
#endif*/
...
/*#if DISABLE_TC_PAR_OLD
    ...
    /*if(!oop(obj_beg)->is_tera_cache() && !oop(obj_beg)->is_tc_to_old()){*/ start_array(id)->allocate_block(obj_beg);//}//tc parallel old
  }
}
                                      ...
                                      bool maximum_compaction)
{
...
#if PARALLEL_MARKING_VECTOR
	std::vector<HeapWord*>::iterator iter;
	std::vector<HeapWord*>::iterator iter_set;
#endif
#if PARALLEL_MARKING_SET
	std::vector<HeapWord*>::iterator iter_set;
	std::set<HeapWord*>::iterator iter;
#endif
  ...
  for (size_t i = 0; i < ParallelGCThreads + 1; ++i) {
    ParCompactionManager* const cm =
      ...
      for(iter = cm->thread_vector.begin(); iter != cm->thread_vector.end(); iter++){
              if(tc_policy(*iter,oop(*iter)->size()) && !tc->tc_check(oop(oop(*iter)->mark()->decode_pointer()))){
                                HeapWord* region_top = (HeapWord*) tc->tc_region_top(oop(*iter), oop(*iter)->size());
                                assertf(tc->tc_check(oop(region_top)), "Pointer from teraCache is not valid");

                                oop(*iter)->forward_to(oop(region_top));

				assertf(oop(*iter)->is_gc_marked(), "Encoding the pointer should preserve the mark.")
#if NEW_FEAT
                                if (Universe::teraCache()->tc_should_mk_dirty(*iter)) {
                                        BarrierSet *bs = Universe::heap()->barrier_set();
                                        ModRefBarrierSet* modBS = (ModRefBarrierSet*)bs;
                                        modBS->tc_write_ref_field(region_top);
                                }
#endif
				tc->po_vector.push_back(*iter);
	      }
      }
  }
#endif
...
#if !DISABLE_TC_PAR_OLD
#if !PARALLEL_MARKING_VECTOR
  std::set<HeapWord*> s( tc->po_vector.begin(), tc->po_vector.end() );
  tc->po_vector.assign( s.begin(), s.end() );
#endif
#endif
      ...
      NOT_PRODUCT(print_generic_summary_data(_summary_data, _space_info));
        ...
        if (EnableTeraCache) {            
                if (TeraCacheStatistics) {           
                        ...
                        phase2 =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000) ;
                ...
                total_phase2 += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms 
                tclog_or_tty->print_cr("TOTAL TIME TAKEN BY PHASE2 = %llu\n", total_phase2);
                }                                                                                                                                    
...
gettimeofday(&start, NULL);
	  ...
	  //std::cout << "num of gcs = " << num_of_gcs << std::endl;
  num_of_gcs++;
  ...
  //Fetching a pointer to a heap instance
  ParallelScavengeHeap* heap = gc_heap();
  ...
  //Fetching a pointer to a young and old gen instance
  PSYoungGen* young_gen = heap->young_gen();
...

  {
    ...
    //time_taken = (time_taken + (end.tv_usec - 
      //                        start.tv_usec)) * 1e-6;
    //std::cout << "Time taken by marking_phase: " << std::fixed
    //     << time_taken;
    //std::cout << " sec" << std::endl;
    //total_phases += time_taken;
   ...
   /* gettimeofday(&end, NULL);
    ...
    time_taken = (time_taken + (end.tv_usec -
                              start.tv_usec)) * 1e-6;*/
    //std::cout << "Time taken by summary_phase: " << std::fixed
      //   << time_taken;
    //std::cout << " sec" << std::endl;
	//total_phases += time_taken;
    ...
    time_taken = (time_taken + (end.tv_usec -
                              start.tv_usec)) * 1e-6;*/
    //std::cout << "Time taken by adjust_phase: " << std::fixed
        // << time_taken;
    //std::cout << " sec" << std::endl;
    //total_phases += time_taken;
    ...
    time_taken = (time_taken + (end.tv_usec -
                              start.tv_usec)) * 1e-6;*/
    //std::cout << "Time taken by compaction_phase: " << std::fixed
        // << time_taken;
    //std::cout << " sec" << std::endl;
    //total_phases += time_taken;

    //std::cout <<"Total time taken by this major gc: " << std::fixed << total_phases << " sec" << std::endl;

    //total_gc_time_par += total_phases;

	//std::cout <<"Total time taken all major gcs: " << std::fixed << total_gc_time_par << " sec" << std::endl;
#if !DISABLE_TC_PAR_OLD
#if REGIONS
    // Print Region Groups
    //Universe::teraCache()->print_region_groups();
  ...
  gettimeofday(&end, NULL);
    ...
    time_taken = (time_taken + (end.tv_usec -
                              start.tv_usec)) * 1e-6;
    total_gc_time_par += time_taken;

        //std::cout <<"Total time taken by all major gcs: " << std::fixed << total_gc_time_par << " sec" << std::endl;
...
// Recursively traverse all live objects and mark them
...
#if DISABLE_LOGGING
  gclog_or_tty->print_cr("Number of gc task threads = %u\n", parallel_gc_threads);
  std::cout << "Number of active workers " << active_gc_threads << std::endl;
#endif
...

  {
    ...
    ParallelScavengeHeap::ParStrongRootsScope psrs;
        ...
        if (EnableTeraCache && !Universe::teraCache()->tc_empty())
                Universe::teraCache()->scavenge(cm);
	//std::cerr << "---END OF SCAVENGE---" << std::endl;
#endif
...
#if DISABLE_LOGGING
	gclog_or_tty->print_cr("\nPARALLEL OLD: count_objs = %u\n", ParCompactionManager::par_count_objs);
	ParCompactionManager::par_count_objs = 0;
#endif
#if !DISABLE_TC_PAR_OLD
  if (EnableTeraCache) {
          ...
          // TeraCache to be enabled to estimate the barriers overhead, we need to
          // unload classes.
  ...
  if (!EnableTeraCache)
  {
        ...
        if (EnableTeraCache && TeraCacheStatistics) {
                gettimeofday(&end_time, NULL);
                        phase1 =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                       
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000) ;

                tclog_or_tty->print_cr("[STATISTICS] | PHASE1 = %llu\n",
                                (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000)); // convert to ms
		total_phase1 += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms
		tclog_or_tty->print_cr("TOTAL TIME TAKEN BY PHASE1 = %llu\n", total_phase1);
	...
	gc_tracer->report_object_count_after_gc(is_alive_closure());
}
...
#if !DISABLE_TC_PAR_OLD
#if PARALLEL_MARKING_SET
void PSParallelCompact::init_par_tc_env(){
	TeraCache *tc = Universe::teraCache();
	size_t set_sz = tc->par_old_set.size();
	unsigned int T = set_sz / (MAX_NUM_THREADS);
	rest = set_sz % (MAX_NUM_THREADS); 
	for(unsigned int t = 0; t < set_sz-rest; t += (set_sz / (MAX_NUM_THREADS))){
                //std::cout << "t = " << t << std::endl;
                //std::cout << "T = " << T << std::endl;
		tc_threads_from.push_back(t);
		tc_threads_to.push_back(T);
		T += set_sz / (MAX_NUM_THREADS); 
	}
}

void thread_adjust(void *pos){
	k++;
	//std::cout << "k = " << k << std::endl;
	unsigned int t = PSParallelCompact::tc_threads_from[k];
	unsigned int T = PSParallelCompact::tc_threads_to[k];
	TeraCache *tc = Universe::teraCache();
        std::set<HeapWord*>::iterator iter = tc->par_old_set.begin();
        std::advance(iter,t);
        std::set<HeapWord*>::iterator end = tc->par_old_set.begin();
        std::advance(end,T);
	for(; iter != end; iter++){
		oop(*iter)->adjust_pointers();
	}
	pthread_barrier_wait(&PSParallelCompact::tc_barrier);
}

void thread_compact(void *pos){
	k++;
        unsigned int t = PSParallelCompact::tc_threads_from[k];
        unsigned int T = PSParallelCompact::tc_threads_to[k];
        TeraCache *tc = Universe::teraCache();
	...
	std::advance(end,T);
        for(; iter != end; iter++){
                  HeapWord *compaction_top = (HeapWord*)oop(*iter)->forwardee();
                  ...
                  if (EnableTeraCache && Universe::teraCache()->tc_check(oop(compaction_top))) {
                          ...
                          memcpy(compaction_top, *iter, oop(*iter)->size() * 8);
...
void PSParallelCompact::init_par_tc_env(){
        TeraCache *tc = Universe::teraCache();
        size_t vec_sz = tc->po_vector.size();
        unsigned int T = vec_sz / (ParallelTCThreads);
        rest = vec_sz % (ParallelTCThreads);
        for(unsigned int t = 0; t < vec_sz-rest; t += (vec_sz / (ParallelTCThreads))){
		//std::cout << "t = " << t << std::endl;
		//std::cout << "T = " << T << std::endl;
                tc_threads_from.push_back(t);
                tc_threads_to.push_back(T);
                T += vec_sz / (ParallelTCThreads);
        }
}
...
void thread_adjust(){
        ...
        unsigned int T = PSParallelCompact::tc_threads_to[k];
        TeraCache *tc = Universe::teraCache();
        std::vector<HeapWord*>::iterator iter = tc->po_vector.begin();
        std::advance(iter,t);
        std::vector<HeapWord*>::iterator end = tc->po_vector.begin();
        std::advance(end,T);
        for(; iter != end; iter++){
                ParCompactionManager* cm = ParCompactionManager::manager_array(0);
                oop(*iter)->update_contents(cm);
                //oop(*iter)->adjust_pointers();
        }
        pthread_barrier_wait(&PSParallelCompact::tc_barrier);
}
...
#endif
void PSParallelCompact::tc_parallel_adjust(){
	for(unsigned int i = 0; i < (ParallelTCThreads); i++){
		unsigned int *x = &i;
		pthread_create(&tc_threads[i], NULL, (void* (*)(void*))thread_adjust, NULL);
	}
}

void PSParallelCompact::tc_parallel_compact(){
        for(unsigned int i = 0; i < (ParallelTCThreads); i++){
                pthread_create(&tc_threads[i], NULL, (void* (*)(void*))thread_compact, NULL);
        }
}
#endif
void PSParallelCompact::adjust_roots() {
  ...
  // Traverse TeraCache
  if (EnableTeraCache && !Universe::teraCache()->tc_empty()){
          Universe::teraCache()->tc_par_adjust(); //parallel old
  }
#endif
...
#if PARALLEL_MARKING_VECTOR
        TeraCache *tc = Universe::teraCache();
        if(tc->po_vector.size() >= ParallelTCThreads){
        	init_par_tc_env();
        	pthread_barrier_init (&tc_barrier, NULL, ParallelTCThreads + 1);
        	tc_parallel_adjust();
        	std::vector<HeapWord*>::iterator iter = tc->po_vector.begin();
        	std::advance(iter,tc_threads_to[ParallelTCThreads-1]);
        	for(; iter != tc->po_vector.end(); iter++){
			ParCompactionManager* cm = ParCompactionManager::manager_array(0);
			oop(*iter)->update_contents(cm);
        	}
        	pthread_barrier_wait(&tc_barrier);
        	k = -1;
        }
        else{
                std::vector<HeapWord*>::iterator iter;
                for(iter = Universe::teraCache()->po_vector.begin(); iter != Universe::teraCache()->po_vector.end(); iter++){
        	        ParCompactionManager* cm = ParCompactionManager::manager_array(0);
	                oop(*iter)->update_contents(cm);
                }
        }
...
#endif
#if PARALLEL_MARKING_SET
                for(iter = Universe::teraCache()->par_old_set.begin(); iter != Universe::teraCache()->par_old_set.end(); iter++){
#endif
#if DEBUG_PAR_OLD_UPDATE_CONTENTS
                        if (EnableTeraCache) {
                        std::cerr << "[ADJUST]"
                                << " | OBJECT = "  << (HeapWord*) oop(*iter)
                                << " | ADDRESS MARK WORD = " << (HeapWord*) oop(*iter)->mark()
				<< " | MARKED = " << PSParallelCompact::is_marked(oop(*iter))
                                << " | STATE = " << oop(*iter)->get_obj_state()
                                << " | SIZE = " << oop(*iter)->size() << std::endl;
                        }
#endif	
			ParCompactionManager* cm = ParCompactionManager::manager_array(0);
                        assertf(Universe::teraCache()->tc_check(oop(oop(*iter)->mark()->decode_pointer())), "should be in tera heap TERA ADJUST");
			oop(*iter)->update_contents(cm);
                }
#endif
#endif
  ...
  PSScavenge::reference_processor()->weak_oops_do(adjust_pointer_closure());
        ...
        if (EnableTeraCache) {                                                          
                if (TeraCacheStatistics) {
                        gettimeofday(&end_time, NULL);               
                        phase3 =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                       
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000) ;

                        tclog_or_tty->print_cr("[STATISTICS] | PHASE3 = %llu\n",
                                        (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000)); // convert to ms
                total_phase3 += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms 
                tclog_or_tty->print_cr("TOTAL TIME TAKEN BY PHASE3 = %llu\n", total_phase3);
...
void PSParallelCompact::compact() {
  ...
  GCTraceTime tm("compaction phase", print_phases(), true, &_gc_timer);
#if PARALLEL_MARKING_SET
  	TeraCache *tc = Universe::teraCache();
	if(tc->par_old_set.size() >= MAX_NUM_THREADS){
		pthread_barrier_init (&tc_barrier, NULL, MAX_NUM_THREADS + 1);
  		tc_parallel_compact();
        	std::set<HeapWord*>::iterator iter = tc->par_old_set.begin();
        	std::advance(iter,tc_threads_to[MAX_NUM_THREADS-1]);
        for(; iter != tc->par_old_set.end(); iter++){
                HeapWord *compaction_top = (HeapWord*)oop(*iter)->forwardee();
                  // Change the value of teraflag in the new location of the object
                  if (EnableTeraCache && Universe::teraCache()->tc_check(oop(compaction_top))/* && oop(*iter)->is_gc_marked()*/) {
                          // Size is in words. Each word is 8 bytes. I use memcpy instead of
                          // memmove to avoid the extra copy of the data in the buffer.
                          memcpy(compaction_top, *iter, oop(*iter)->size() * 8);
                          oop(compaction_top)->set_obj_in_tc();
	...
	pthread_barrier_init (&tc_barrier, NULL, ParallelTCThreads + 1);
        tc_parallel_compact();
	//std::cout << "t init = " << tc->po_vector.size() - rest << std::endl;
        std::vector<HeapWord*>::iterator iter = tc->po_vector.begin();
        std::advance(iter,tc_threads_to[ParallelTCThreads-1]);
        for(; iter != tc->po_vector.end(); iter++){
		//std::cout<< "t = " << t << std::endl;
...
#endif
			HeapWord *compaction_top = (HeapWord*)oop(*iter)->forwardee();
#if DEBUG_PAR_OLD_COMPACT
                  if (EnableTeraCache) {
                          std::cerr << "[COMPACT] | "    << "O = " << *iter
                                                << " | MARK = "      << oop(*iter)->mark()
						<< " | MARKED = " << PSParallelCompact::is_marked(oop(*iter))
                                                << " | STATE = "     << oop(*iter)->get_obj_state();
                  }
#endif
			//if(PSParallelCompact::is_marked(oop(*iter))) std::cerr << "Marked in tera compact" << std::endl;
			assertf(Universe::teraCache()->tc_check(oop(compaction_top)), "should be in tera heap TERA COMPACT");
                  // Change the value of teraflag in the new location of the object
                  if (EnableTeraCache && Universe::teraCache()->tc_check(oop(compaction_top))/* && oop(*iter)->is_gc_marked()*/) {
                          // Size is in words. Each word is 8 bytes. I use memcpy instead of
                          // memmove to avoid the extra copy of the data in the buffer.
                          memcpy(compaction_top, *iter, oop(*iter)->size() * 8);
			  //std::cout << "moved to tera" << std::endl;
                          oop(compaction_top)->set_obj_in_tc();
                          oop(compaction_top)->init_mark();
                  }
...
#endif
#if PARALLEL_MARKING_SET
		std::cout << "AFTER COMPACT: par_old_set size = " << Universe::teraCache()->par_old_set.size() << std::endl;
#endif
#if PARALLEL_MARKING_VECTOR
		//std::cout << "AFTER COMPACT: po_vector size = " << Universe::teraCache()->po_vector.size() << std::endl;
#endif
#endif
...
#if !DISABLE_TC_PAR_OLD 
#if PARALLEL_MARKING_VECTOR
  Universe::teraCache()->po_vector.clear();
#endif
#if PARALLEL_MARKING_SET
  Universe::teraCache()->par_old_set.clear();
#endif
#if !PS_LOCAL_LOCK
  for (size_t i = 0; i < ParallelGCThreads + 1; ++i) {
    ParCompactionManager* const cm =
      ParCompactionManager::manager_array(int(i));
      cm->thread_vector.clear();
  }
#endif
#if PARALLEL_TC
  tc_threads_from.clear();
  tc_threads_to.clear();
#endif
#endif
        ...
        if (EnableTeraCache) {
                if (TeraCacheStatistics) {
                        gettimeofday(&end_time, NULL);
                        phase4 =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                       
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000) ;

                        tclog_or_tty->print_cr("[STATISTICS] | PHASE4 = %llu\n",
                                        (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000)); // convert to ms
                total_phase4 += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms 
                tclog_or_tty->print_cr("TOTAL TIME TAKEN BY PHASE4 = %llu\n", total_phase4);
		total_major_gcs += phase1 + phase2 + phase3 + phase4;
		tclog_or_tty->print_cr("TOTAL TIME TAKEN BY ALL MAJOR GCS = %llu\n", total_major_gcs);
                }
...
void PSParallelCompact::update_deferred_objects(ParCompactionManager* cm,
                                                SpaceId id) {
  assertf(id < last_space_id, "bad space id");
  ...
  assertf(space_info->dense_prefix() >= space->bottom(), "dense_prefix not set");
...
#if !DISABLE_TC_PAR_OLD
      //map_monitor->lock();
      if(Universe::teraCache()->tc_check(oop(addr))) std::cout << "deferred in tera" << std::endl;
      if(!oop(addr)->is_tera_cache())oop(addr)->update_contents(cm);
      if(oop(addr)->is_tera_cache())oop(addr)->set_obj_state();
      if(oop(addr)->mark()->is_marked()) oop(addr)->init_mark();
     // map_monitor->unlock();
#else
      oop(addr)->update_contents(cm);
#endif

      assertf(oop(addr)->is_oop_or_null(), "should be an oop now");
    }
  }
}
...
{
	//PSParallelCompact::tmap_monitor->lock();
  ...
  assertf(dest_addr < new_top, "sanity");
  ...
  do {
    ...
    if (status == ParMarkBitMap::incomplete) {
      ...
      if (obj_end < range_end) {
        ...
        assertf(status != ParMarkBitMap::would_overflow, "sanity");
      } else {
        // The end was not found; the object will not fit.
        assertf(range_end < src_space_top, "obj cannot cross space boundary");
  ...
  //PSParallelCompact::tmap_monitor->unlock();
}
...
MoveAndUpdateClosure::do_addr(HeapWord* addr, size_t words) {
  assertf(destination() != NULL, "sanity");
  assertf(bitmap()->obj_size(addr) == words, "bad size");
  _source = addr;
  assertf(PSParallelCompact::summary_data().calc_new_pointer(source()) ==
  ...
  if (destination() != source()) {
    	DEBUG_ONLY(PSParallelCompact::check_new_location(source(), destination());)
	Copy::aligned_conjoint_words(source(), destination(), words); //tc parallel old
  }
#if !DISABLE_TC_PAR_OLD
  	if(!moved_oop->is_tera_cache())moved_oop->update_contents(compaction_manager()); 
        if(moved_oop->is_tera_cache()) moved_oop->set_obj_state();
        if(moved_oop->mark()->is_marked()) moved_oop->init_mark();
#else 
	moved_oop->update_contents(compaction_manager());
#endif
  assertf(moved_oop->is_oop_or_null(), "Object should be whole at this point");
  
  update_state(words);
  assertf(destination() == (HeapWord*)moved_oop + moved_oop->size(), "sanity");
