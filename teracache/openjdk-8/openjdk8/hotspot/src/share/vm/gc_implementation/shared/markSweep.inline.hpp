		...
		if (!obj->mark()->is_marked()) {
			...
			if (!(obj->mark()->is_marked() && obj->is_tera_cache())) {
				...
				if (!obj->is_tera_cache()){
					//gclog_or_tty->print_cr("\nSERIAL OLD: set_tera_cache\n");
					//std::cout << "serial tera" << std::endl;
...
template <class T> inline void MarkSweep::adjust_pointer(oop start_p, T* p) {
	T heap_oop = oopDesc::load_heap_oop(p);
	if (!oopDesc::is_null(heap_oop) ) {
		oop obj     = oopDesc::decode_heap_oop_not_null(heap_oop);
...
#if REGIONS
            //Universe::teraCache()->group_region_enabled((HeapWord*) new_obj, NULL, NULL, (HeapWord*)p);
                if(start_p != NULL){
