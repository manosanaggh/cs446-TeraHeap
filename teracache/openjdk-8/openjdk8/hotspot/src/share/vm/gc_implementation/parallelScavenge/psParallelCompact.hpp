...
class PSParallelCompact : AllStatic {
 ...
 private:
  ...
  static pthread_t		tc_threads[MAX_NUM_THREADS]; //parallel old
  //static Monitor		*map_monitor;
...
#endif  // #ifdef ASSERT
  public:
  static std::vector<unsigned int> tc_threads_from; //parallel old
  static std::vector<unsigned int> tc_threads_to; //parallel old
  static pthread_barrier_t      tc_barrier; //parallel old
  static Monitor              *map_monitor;
 private:
  //tc parallel functions
  static void init_par_tc_env();
  static void tc_parallel_adjust();
  static void tc_parallel_compact();
 ...
 public:
  ...
  template <class T> static inline void mark_and_push(ParCompactionManager* cm,
                                                      T* p);
  template <class T> static inline void adjust_pointer(oop start_p, T* p);

  static inline void follow_klass(ParCompactionManager* cm, Klass* klass);

  template <class T> static inline void tera_mark_and_push(ParCompactionManager *cm, T* p);//
  ...
  static bool tc_policy(HeapWord *q, size_t size);
#endif
  ...
  if (!oopDesc::is_null(heap_oop)) {
    oop obj = oopDesc::decode_heap_oop_not_null(heap_oop);
            ...
            Universe::teraCache()->mark_used_region((HeapWord*)obj);
                        ...
                        if (EnableTeraCache)
                        {
                                std::cerr <<"[PAR_MARK_AND_PUSH POLICY in h2]"
                                        << " | P = " << p
                                        << " | OBJECT = "
                                        << (HeapWord*)obj;
				std::cerr
	...
	map_monitor->lock();
        /*if(obj->is_tera_cache() && is_marked(obj) && !tc->tc_check(obj->forwardee())){
                std::cerr << "already tf and marked and not forwarded. MP" << std::endl;
        }*/

#endif
	if(obj->is_tera_cache() && !Universe::teraCache()->tc_check(oop(obj->mark()->decode_pointer())) && mark_bitmap()->is_unmarked(obj)){
#if PS_LOCAL_LOCK
			MarkSweep::mark_object(obj);
                	HeapWord* region_top = (HeapWord*) tc->tc_region_top(obj, obj->size());
			assertf(tc->tc_check(oop(region_top)), "Pointer from teraCache is not valid");

                        obj->forward_to(oop(region_top));
#if NEW_FEAT
			if (Universe::teraCache()->tc_should_mk_dirty((HeapWord*)obj)) {
				BarrierSet *bs = Universe::heap()->barrier_set();
				ModRefBarrierSet* modBS = (ModRefBarrierSet*)bs;															modBS->tc_write_ref_field(region_top);
			}
#endif
			Universe::teraCache()->po_vector.push_back((HeapWord*)obj);
#endif
#if !PS_LOCAL_LOCK
			        //map_monitor->lock();
				MarkSweep::mark_object(obj);
                                cm->thread_vector.push_back((HeapWord*)obj);
				//map_monitor->unlock();
#endif
    ...
    if (mark_bitmap()->is_unmarked(obj) && mark_obj(obj)) {
      cm->push(obj);
                        ...
                        if (EnableTeraCache)
                        {
                                std::cerr <<"[PAR_MARK_AND_PUSH POLICY]"
                                        << " | P = " << p
                                        << " | OBJECT = "
                                        << (HeapWord*)obj;
				std::cerr 
...
template <class T> inline void PSParallelCompact::tera_mark_and_push(ParCompactionManager *cm, T* p) {
        T heap_oop = oopDesc::load_heap_oop(p);
...
#if DEBUG_PAR_OLD_MARKING
                        if (EnableTeraCache) {
                                std::cerr <<"[PAR_TERA_MARK_AND_PUSH]"
                                        << " | P = " << p
                                        << " | OBJECT = " << (HeapWord*)obj
                                        << " | MARKED = " << is_marked(obj)
                                        << " | TERA = "   << obj->get_obj_state()
                                        << " | KLASS = "  << obj->klass()
                                        << "| NEW ADDRESS (MUST BE GARBAGE)= " << obj->mark()->decode_pointer()
                                        << std::endl;
                        }
#endif			
                        return;
...
#if DEBUG_PAR_OLD_MARKING
                        if (EnableTeraCache) {
                                std::cerr <<"[PAR_TERA_MARK_AND_PUSH BACK_PTR]"
                                        << " | P = " << p
                                        << " | OBJECT = " << (HeapWord*)obj
                                        << " | MARKED = " << is_marked(obj)
                                        << " | TERA = "   << obj->get_obj_state()
...
map_monitor->lock();
#endif
	if(obj->is_tera_cache() && !Universe::teraCache()->tc_check(obj->forwardee())){
#if PS_LOCAL_LOCK //                                                   
						MarkSweep::mark_object(obj);
                                                HeapWord* region_top = (HeapWord*) tc->tc_region_top(obj, obj->size());
                                                assertf(tc->tc_check(oop(region_top)), "Pointer from teraCache is not valid");
        ...
        map_monitor->unlock();
			...
			if(mark_bitmap()->is_unmarked(obj) || !obj->is_tera_cache()){
					if(mark_bitmap()->is_unmarked(obj)) mark_obj(obj);
#if PS_LOCAL_LOCK //
        map_monitor->lock();
#endif
					...
					cm->push(obj);
                        ...
                        if (EnableTeraCache) {
                                std::cerr <<"[PAR_TERA_MARK_AND_PUSH]"
                                        ...
                                        << " | TERA = "   << obj->get_obj_state()
					<< " | KLASS = "  << obj->klass()
					<< "| NEW ADDRESS = " << obj->mark()->decode_pointer()
                                        << std::endl;
                        }
#endif
		}
...
template <class T>
inline void PSParallelCompact::adjust_pointer(oop start_p, T* p) {
  T heap_oop = oopDesc::load_heap_oop(p);
  if (!oopDesc::is_null(heap_oop)) {
    ...
    oop obj     = oopDesc::decode_heap_oop_not_null(heap_oop);
        assertf(Universe::heap()->is_in(obj) ||
                ...
                else if(obj->is_tera_cache()) {
			new_obj = oop(obj->mark()->decode_pointer());
			if(!Universe::teraCache()->tc_check(new_obj)){
				if(mark_bitmap()->is_unmarked(obj)) mark_obj(obj);
				new_obj = (oop)summary_data().calc_new_pointer(obj);
			        assertf(start_p == NULL || Universe::heap()->is_in(oop(start_p)) ||
                                Universe::teraCache()->tc_check(oop(start_p)), "obj parent should be in heap");

                                assertf(Universe::heap()->is_in(obj), "obj of field should be in heap");
...
#if REGIONS
  		if(start_p != NULL){
  			oop new_obj_parent = NULL;

  			new_obj_parent = oop(start_p->mark()->decode_pointer());

            		Universe::teraCache()->group_region_enabled((HeapWord*) new_obj, (HeapWord*)start_p, (HeapWord*)new_obj_parent, (HeapWord*)p);
		}
#endif
...
class MoveAndUpdateClosure: public ParMarkBitMapClosure {
 ...
 protected:
  ...
  inline void update_state_tera(size_t words_count);
...
inline void MoveAndUpdateClosure::update_state_tera(size_t words){
	decrement_words_remaining(words);
	_source += words;
}
...
#if !DISABLE_TC_PAR_OLD
  //PSParallelCompact::map_monitor->lock();
  if(!oop(addr)->is_tera_cache())oop(addr)->update_contents(compaction_manager());
  if(oop(addr)->is_tera_cache()) oop(addr)->set_obj_state();
  if(oop(addr)->mark()->is_marked()) oop(addr)->init_mark();
  //PSParallelCompact::map_monitor->unlock();
#else
