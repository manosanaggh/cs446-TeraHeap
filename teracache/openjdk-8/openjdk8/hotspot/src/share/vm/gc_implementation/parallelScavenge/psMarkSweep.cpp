...
unsigned int num_of_gcs_2 = 1;
unsigned long long total_phase1_=0,total_phase2_=0,total_phase3_=0, total_phase4_=0, phase1_=0, phase2_=0, phase3_=0,phase4_=0, total_major_gcs_=0;
...
gettimeofday(&start, NULL);
          ...
          //std::cout << "num of gcs = " << num_of_gcs_2 << std::endl;
  num_of_gcs_2++;
...

  {
    ...
    //time_taken = (time_taken + (end.tv_usec -
      //                        start.tv_usec)) * 1e-6;
    //std::cout << "Time taken by precompaction_phase: " << std::fixed
         //<< time_taken;
    //std::cout << " sec" << std::endl;
    //total_phases += time_taken;
    ...
    //total_gc_time += total_phases;

      //  std::cout <<"Total time taken all major gcs: " << std::fixed << total_gc_time << " sec" << std::endl;
  ...
  gettimeofday(&end, NULL);
    ...
    time_taken = (time_taken + (end.tv_usec -
                              start.tv_usec)) * 1e-6;
    total_gc_time += time_taken;

        //std::cout <<"Total time taken by all major gcs: " << std::fixed << total_gc_time << " sec" << std::endl;
...
#if DISABLE_LOGGING 
        gclog_or_tty->print_cr("\nSERIAL OLD: count_objs = %u\n", MarkSweep::count_objs);
	MarkSweep::count_objs = 0;
#endif
	  ...
	  SymbolTable::unlink();
        ...
        if (EnableTeraCache && TeraCacheStatistics) {                             
                gettimeofday(&end_time, NULL);                                            
                        phase1_ =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                       
                ...
                total_phase1_ += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms 
...
void PSMarkSweep::mark_sweep_phase2() {
  ...
  old_gen->precompact();
        ...
        if (EnableTeraCache && TeraCacheStatistics) {                             
                gettimeofday(&end_time, NULL);                                            
                        phase2_ =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                       
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000) ;                                             
                tclog_or_tty->print_cr("[STATISTICS] | PHASE2 = %llu\n",                                       
                                (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms        
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000)); // convert to ms        
                total_phase2_ += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms 
...
void PSMarkSweep::mark_sweep_phase3() {
  ...
  old_gen->adjust_pointers();
        ...
        if (EnableTeraCache && TeraCacheStatistics) {                             
                gettimeofday(&end_time, NULL);                                            
                        phase3_ =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                       
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000) ;                                             
                tclog_or_tty->print_cr("[STATISTICS] | PHASE3 = %llu\n",                                       
                                (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms        
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000)); // convert to ms        
                total_phase3_ += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms 
...
void PSMarkSweep::mark_sweep_phase4() {
  ...
  young_gen->compact();
        ...
        if (EnableTeraCache && TeraCacheStatistics) {                             
                gettimeofday(&end_time, NULL);                                           
                        phase4_ =  (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                       
                                        (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000) ; 

                tclog_or_tty->print_cr("[STATISTICS] | PHASE4 = %llu\n",                                       
                                (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms        
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000)); // convert to ms        
                total_phase4_ += (unsigned long long)((end_time.tv_sec - start_time.tv_sec) * 1000) + // convert to ms                                
                                (unsigned long long)((end_time.tv_usec - start_time.tv_usec) / 1000); // convert to ms 
                tclog_or_tty->print_cr("TOTAL TIME TAKEN BY PHASE4 = %llu\n", total_phase4_);
                total_major_gcs_ += phase1_ + phase2_ + phase3_ + phase4_;                                                        
