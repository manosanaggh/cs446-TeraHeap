...
class TeraCache{
	...
	public:
		...
		void scavenge(ParCompactionManager *cm); //
		...
		std::vector <HeapWord*> po_vector; // parallel old vector

		std::set <HeapWord*> par_old_set; // parallel old set

		//std::vector <oop> field_parents;

		Stack<HeapWord*, mtGC> _tc_par_old_stack; // parallel old stack
        ...
        void group_region_enabled(HeapWord *new_obj_addr, HeapWord *h1_obj_addr, HeapWord *h2_obj_addr, HeapWord *h1_field_addr);
