...
/*scavenge() version for parallel old gc.*/
void TeraCache::scavenge(ParCompactionManager *cm)
{
        struct timeval start_time;
        struct timeval end_time;
        ...
        while (!_tc_stack.is_empty()) {
                ...
                if (TeraCacheStatistics)
                        back_ptrs_per_fgc++;
#if DEBUG_TC_SCAVENGE
                        if (EnableTeraCache)                                
                        {                 
                                std::cerr <<"[TC_SCAVENGE]"
                                        << " | P = " << &obj                
                                        << " | OBJECT = "                                                            
                                        << (HeapWord*)obj                                                              
                                        << " | MARKED = "               
                                        << PSParallelCompact::is_marked(obj)                                  
                                        << " | TERA = "
                                        << obj->get_obj_state()
                                        << " | KLASS = "  << obj->klass()
                                        << " | NEW ADDRESS (MUST BE GARBAGE)= "
                                        << obj->mark()->decode_pointer()
                                        << std::endl;
                        }
#endif
			//assertf(!PSParallelCompact::is_marked(obj),"[TC SCAVENGE] Back pointer should not be marked.");
			/*if(PSParallelCompact::is_marked(obj)) cm->push(obj);
			if(obj->is_tera_cache() && !Universe::teraCache()->tc_check(oop(obj->mark()->decode_pointer())) && PSParallelCompact::is_marked(obj)){
                                MarkSweep::mark_object(obj);
                                cm->thread_vector.push_back((HeapWord*)obj);
			}*/
                PSParallelCompact::mark_and_push(cm,&obj);
#if DEBUG_TC_SCAVENGE
                        if (EnableTeraCache)
                        {
                                std::cerr <<"[TC_SCAVENGE]"
...
void TeraCache::tc_adjust() {
	...
	while (!_tc_adjust_stack.is_empty()) {
		...
		MarkSweep::adjust_pointer(NULL,obj);
...
/*tc_adjust() version for parallel old gc.*/
void TeraCache::tc_par_adjust() {
        struct timeval start_time;
        struct timeval end_time;
	unsigned int i = 0;
        ...
        Universe::teraCache()->enable_groups((HeapWord*) obj);
                        if (EnableTeraCache) {
                                //std::cerr <<"[PAR_TERA_MARK_AND_PUSH]"
                                        //<< " | P = " << p
                                        //<< " | OBJECT = " << (HeapWord*)obj
                                        //<< " | MARKED = " << PSParallelCompact::is_marked(obj)
                ...
                PSParallelCompact::adjust_pointer(NULL, obj);
	//	i++;
        Universe::teraCache()->disable_groups();
        }
...
// are grouped 
void TeraCache::group_region_enabled(HeapWord *new_obj_addr, HeapWord *h1_obj_addr, HeapWord *h2_obj_addr, HeapWord *h1_field_addr){
#if MARK_TERA_CT_FIX
    //assertf(new_obj_addr != NULL, "new_obj_addr is NULL.");
    //if(!new_obj_addr) return;
    assertf(h1_field_addr != NULL, "h1_field_addr is NULL.");
#endif
    check_for_group((char*) new_obj_addr);

#if MARK_TERA_CT_FIX
    if(h2_obj_addr != NULL){
    if(!tc_check(oop(h1_obj_addr)) && tc_check(oop(h2_obj_addr)) && !tc_check(oop(new_obj_addr))){
#if !DISABLE_LOGGING
	    //std::cerr << "group_region_enabled" << std::endl;
#endif
    BarrierSet *bs = Universe::heap()->barrier_set();

    if(bs->is_a(BarrierSet::ModRef)){
	ModRefBarrierSet *modBS = (ModRefBarrierSet*)bs;
	size_t diff = oop(h1_obj_addr)->get_field_offset(h1_field_addr);
	assertf(diff > 0 && (diff <= (uint64_t) oop(h1_obj_addr)->size()), "Diff out of range: %lu", diff);
	HeapWord *h2_field_addr = h2_obj_addr + diff;
	assertf(tc_is_in((void*) h2_field_addr), "Should be in H2");
	modBS->tc_write_ref_field(h2_field_addr);
    }
    }
    }
#endif
