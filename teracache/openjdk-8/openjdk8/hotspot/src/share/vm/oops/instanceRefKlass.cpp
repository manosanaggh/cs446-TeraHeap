...
#if TEST_CLOSURE
                  if (EnableTeraCache && obj->is_tera_cache())
                          PSParallelCompact::tera_mark_and_push(cm,referent_addr);
                  else
  ...
  if (ReferenceProcessor::pending_list_uses_discovered_field()) {
      ...
      debug_only(
        if(TraceReferenceGC && PrintGCDetails) {
          gclog_or_tty->print_cr("   Process discovered as normal "
                                ...
                                {
                                        printf("SetTeraCache\n");
                                        PSParallelCompact::tera_mark_and_push(cm,discovered_addr);
                                }
                                else {
    ...
    assert(oopDesc::is_null(next) || oopDesc::is_null(discovered),
           err_msg("Found an inactive reference " PTR_FORMAT " with a non-NULL discovered field",
                ...
                // Process the next attribute
                if (EnableTeraCache && obj->is_tera_cache())
                        PSParallelCompact::tera_mark_and_push(cm,next_addr);
                else
...
template <class T> void specialized_oop_adjust_pointers(InstanceRefKlass *ref, oop obj) {
  ...
  T* referent_addr = (T*)java_lang_ref_Reference::referent_addr(obj);
  MarkSweep::adjust_pointer(obj,referent_addr);

  T* next_addr = (T*)java_lang_ref_Reference::next_addr(obj);
  MarkSweep::adjust_pointer(obj,next_addr);

  T* discovered_addr = (T*)java_lang_ref_Reference::discovered_addr(obj);
  MarkSweep::adjust_pointer(obj,discovered_addr);
  if (EnableTeraCache && obj->is_tera_cache()){                  \
...
void specialized_oop_update_pointers(InstanceRefKlass *ref,
  ...
  T* referent_addr = (T*)java_lang_ref_Reference::referent_addr(obj);
  PSParallelCompact::adjust_pointer(obj,referent_addr);
  T* next_addr = (T*)java_lang_ref_Reference::next_addr(obj);
  PSParallelCompact::adjust_pointer(obj,next_addr);
  T* discovered_addr = (T*)java_lang_ref_Reference::discovered_addr(obj);
  PSParallelCompact::adjust_pointer(obj,discovered_addr);
  if (EnableTeraCache && obj->is_tera_cache()){                  
      Universe::teraCache()->disable_groups();                   
  }                                                              
