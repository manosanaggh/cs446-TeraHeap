...
#define ObjArrayKlass_SPECIALIZED_UPDATE_OOP_ITERATE(obj, T, a, p, do_oop) \
{                                       \
  T* p         = (T*)(a)->base();   \
  T* const end = p + (a)->length(); \
  while (p < end) {                 \
...
#define ObjArrayKlass_OOP_ITERATE(obj, a, p, do_oop)      \
  if (UseCompressedOops) {                           \
    ObjArrayKlass_SPECIALIZED_OOP_ITERATE(narrowOop, \
      a, p, do_oop)                                  \
  } else {                                           \
    ObjArrayKlass_SPECIALIZED_UPDATE_OOP_ITERATE(obj, oop,       \
      a, p, do_oop)                                  \
  }
...
void ObjArrayKlass::oop_follow_contents(ParCompactionManager* cm,
   ...
   if(obj == NULL) std::cout << "obj NULL" << std::endl;
  assertf(obj->is_array(), "obj must be array");
...
int ObjArrayKlass::oop_oop_iterate##nv_suffix(oop obj,                          \
  ...
  ObjArrayKlass_OOP_ITERATE(obj, a, p, (closure)->do_oop##nv_suffix(p))              \
  return size;                                                                  \
}
...
int ObjArrayKlass::oop_adjust_pointers(oop obj) {
  ...
  ObjArrayKlass_OOP_ITERATE(obj, a, p, MarkSweep::adjust_pointer(obj, p))
  if (EnableTeraCache && obj->is_tera_cache()){                  
...
int ObjArrayKlass::oop_update_pointers(ParCompactionManager* cm, oop obj) {
  ...
  ObjArrayKlass_OOP_ITERATE(obj, a, p, PSParallelCompact::adjust_pointer(obj, p))
  if (EnableTeraCache && obj->is_tera_cache()){
      Universe::teraCache()->disable_groups();
  }
  return size;
