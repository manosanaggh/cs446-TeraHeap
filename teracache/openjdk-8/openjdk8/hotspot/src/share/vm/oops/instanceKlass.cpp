...
#define InstanceKlass_SPECIALIZED_UPDATE_OOP_ITERATE( \
  obj, T, start_p, count, do_oop,                \
  assert_fn)                                \
{                                               \
  T* p         = (T*)(start_p);             \
  T* const end = p + (count);               \
...
#define InstanceKlass_PARALLEL_TC_OOP_MAP_ITERATE(cm, obj, do_oop, assert_fn)                  \
{                                                                                 \
  /* Compute oopmap block range. The common case                                  \
     is nonstatic_oop_map_size == 1. */                                           \
  OopMapBlock* map           = start_of_nonstatic_oop_maps();                     \
      ...
      ++map;                                                                      \
          ...
          while (map < end_map) {                                                 \
                  if (TestBit(_transient_field_bitmap, i)) {                          \
                          InstanceKlass_SPECIALIZED_OOP_ITERATE(oop,                      \
                                          obj->obj_field_addr<oop>(map->offset()), map->count(),  \
                                          PSParallelCompact::mark_and_push(cm,p), assert_fn)                 \
                  }                                                                                                                                   \
                  else {                                                                                                                      \
                          InstanceKlass_SPECIALIZED_OOP_ITERATE(oop,                      \
                                          obj->obj_field_addr<oop>(map->offset()), map->count(),  \
                                          PSParallelCompact::tera_mark_and_push(cm,p), assert_fn)            \
                  }                                                                                                                                   \
                  ++i;                                                                \
                  ++map;                                                              \
          }                                                                       \
...
{                                                                        \
  ...
  if (UseCompressedOops) {                                               \
    while (map < end_map) {                                              \
	  ...
	  while (map < end_map) {                                            \
		  InstanceKlass_SPECIALIZED_UPDATE_OOP_ITERATE(obj, oop,                     \
				  obj->obj_field_addr<oop>(map->offset()), map->count(), \
				  do_oop, assert_fn)                                     \
		  ++map;                                                         \
	  }                                                                  \
...
void InstanceKlass::oop_follow_contents(ParCompactionManager* cm,
  ...
  // everything else.
        ...
        if (EnableTeraCache && obj->is_tera_cache())
        {
                Universe::teraCache()->set_cur_obj_group_id((long int) obj->get_obj_group_id());
#if P_SD
                InstanceKlass_PARALLEL_TC_OOP_MAP_ITERATE( \
				cm, \
                                obj, \
                                PSParallelCompact::tera_mark_and_push(cm,p), \
                                assert_is_in)
#else
                InstanceKlass_PARALLEL_TC_OOP_MAP_ITERATE( \
				cm, \
...
int InstanceKlass::oop_adjust_pointers(oop obj) {
  ...
  InstanceKlass_OOP_MAP_ITERATE( \
    obj, \
    MarkSweep::adjust_pointer(obj,p), \
    assert_is_in)
  return size;
}
...
int InstanceKlass::oop_update_pointers(ParCompactionManager* cm, oop obj) {
  ...
  InstanceKlass_OOP_MAP_ITERATE( \
    obj, \
    PSParallelCompact::adjust_pointer(obj, p), \
    assert_is_in)
  return size;
}
